                            ...
                            int oppenc_mode)
{
  ...
  mutt_log (5, "pgp_getkeybyaddr: looking for %s <%s>.",
...
pgp_key_t pgp_getkeybystr (char *p, short abilities, pgp_ring_t keyring)
{
  ...
  for (k = keys; k; k = kn)
  {
    ...
    mutt_log (5, "pgp_getkeybystr: matching \"%s\" against key %s:\n",
                p, pgp_long_keyid (k));
    ...
    else
    {
      ...
      {
        mutt_log (5, "pgp_getkeybystr: matching \"%s\" against key %s, \"%s\":\n",
