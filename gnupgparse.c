...
static pgp_key_t parse_pub_line (char *buf, int *is_subkey, pgp_key_t k)
{
  ...
  *is_subkey = 0;
  ...
  mutt_log (2, "parse_pub_line: buf = `%s'\n", buf);
  ...
  for (p = buf; p; p = pend)
  {
	...
	mutt_log (2, "capabilities info: %s\n", p);
...
bail:
  mutt_log (5, "parse_pub_line: invalid number: '%s'\n", p);
  return NULL;
}
