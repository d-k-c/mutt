...
int imap_access (const char* path, int flags)
{
  ...
  {
    mutt_log (3, "imap_access: found %s in cache\n", mailbox);
    return 0;
  }
  ...
  {
    mutt_log (2, "imap_access: STATUS not supported?\n");
    return -1;
  }

  if ((rc = imap_exec (idata, buf, IMAP_CMD_FAIL_OK)) < 0)
  {
    mutt_log (1, "imap_access: Can't check STATUS of %s\n", mbox);
    return rc;
  }
...
int imap_read_literal (FILE* fp, IMAP_DATA* idata, long bytes, progress_t* pbar)
{
  ...
  mutt_log (2, "imap_read_literal: reading %ld bytes\n", bytes);

  for (pos = 0; pos < bytes; pos++)
  {
    if (mutt_socket_readchar (idata->conn, &c) != 1)
    {
      mutt_log (1, "imap_read_literal: error during read, %ld bytes read\n", pos);
...
void imap_expunge_mailbox (IMAP_DATA* idata)
{
  ...
  for (i = 0; i < idata->ctx->msgcount; i++)
  {
    ...
    if (h->index == -1)
    {
      mutt_log (2, "Expunging message UID %d.\n", HEADER_DATA (h)->uid);
...
IMAP_DATA* imap_conn_find (const ACCOUNT* account, int flags)
{
  ...
  if (idata->state == IMAP_CONNECTED)
  {
    if (!imap_authenticate (idata))
    {
      ...
      new = 1;
      if (idata->conn->ssf)
	mutt_log (2, "Communication encrypted at %d bits\n",
		    idata->conn->ssf);
    }
    else
...
static char* imap_get_flags (LIST** hflags, char* s)
{
  ...
  {
    mutt_log (1, "imap_get_flags: not a FLAGS response: %s\n",
      s);
    return NULL;
  }
  s += 5;
  ...
  if (*s != '(')
  {
    mutt_log (1, "imap_get_flags: bogus FLAGS response: %s\n",
      s);
    return NULL;
  }
...
static int imap_open_mailbox (CONTEXT* ctx)
{
  ...
  do
  {
    ...
    if (ascii_strncasecmp ("FLAGS", pc, 5) == 0)
    {
      ...
      if (!idata->flags)
      {
	mutt_log (3, "Getting mailbox FLAGS\n");
    ...
    {
      mutt_log (3, "Getting mailbox PERMANENTFLAGS\n");
    ...
    {
      mutt_log (3, "Getting mailbox UIDVALIDITY\n");
      pc += 3;
    ...
    {
      mutt_log (3, "Getting mailbox UIDNEXT\n");
      pc += 3;
      pc = imap_next_word (pc);
  ...
  {
    mutt_log (2, "Mailbox is read-only.\n");
    ctx->readonly = 1;
  }
  ...
  {
    if (!idata->flags)
      mutt_log (3, "No folder flags found\n");
    else
    {
      LIST* t = idata->flags;

      mutt_log (3, "Mailbox flags: ");
  ...
  mutt_log (2, "imap_open_mailbox: msgcount is %d\n", ctx->msgcount);
  FREE (&mx.mbox);
  return 0;
                      ...
                      int flag, int changed, int invert)
{
  ...
  {
    mutt_log (1, "imap_exec_msgset: unable to allocate buffer\n");
    return -1;
  }
...
int imap_sync_mailbox (CONTEXT* ctx, int expunge, int* index_hint)
{
  ...
  if (idata->state < IMAP_SELECTED)
  {
    mutt_log (2, "imap_sync_mailbox: no mailbox selected\n");
    return -1;
  }
  ...
  for (n = 0; n < ctx->msgcount; n++)
  {
	...
	if (!appendctx)
	  mutt_log (1, "imap_sync_mailbox: Error opening mailbox in append mode\n");
	else
...
int imap_check_mailbox (CONTEXT *ctx, int *index_hint, int force)
{
  ...
  if (idata->state == IMAP_IDLE)
  {
    while ((result = mutt_socket_poll (idata->conn)) > 0)
    {
      if (imap_cmd_step (idata) != IMAP_CMD_CONTINUE)
      {
        mutt_log (1, "Error reading IDLE response\n");
        return -1;
      }
    }
    if (result < 0)
    {
      mutt_log (1, "Poll failed, disabling IDLE\n");
...
static int imap_get_mailbox (const char* path, IMAP_DATA** hidata, char* buf, size_t blen)
{
  ...
  if (imap_parse_path (path, &mx))
  {
    mutt_log (1, "imap_get_mailbox: Error parsing %s\n", path);
    return -1;
  }
...
int imap_buffy_check (int force, int check_stats)
{
  ...
  for (mailbox = Incoming; mailbox; mailbox = mailbox->next)
  {
    ...
    {
      mutt_log (2, "Server doesn't support STATUS\n");
      continue;
    }
    ...
    if (lastdata && idata != lastdata)
    {
      ...
      if (imap_exec (lastdata, NULL, IMAP_CMD_FAIL_OK) == -1)
        mutt_log (1, "Error polling mailboxes\n");
    ...
    {
      mutt_log (1, "Error queueing command\n");
      return 0;
    }
  }
...
IMAP_STATUS* imap_mboxcache_get (IMAP_DATA* idata, const char* mbox, int create)
{
  ...
  if (hc)
  {
    ...
    if (uidvalidity)
    {
      ...
      mutt_log (3, "mboxcache: hcache uidvalidity %d, uidnext %d\n",
...
int imap_subscribe (char *path, int subscribe)
{
  ...
  if (option (OPTIMAPCHECKSUBSCRIBED))
  {
    ...
    if (mutt_parse_rc_line (mbox, &token, &err))
      mutt_log (1, "Error adding subscribed mailbox: %s\n", errstr);
    FREE (&token.data);
  }
...
int imap_fast_trash (CONTEXT* ctx, char* dest)
{
  ...
  {
    mutt_log (1, "imap_fast_trash: bad destination %s\n", dest);
    return -1;
  }
  ...
  {
    mutt_log (3, "imap_fast_trash: %s not same server as %s\n",
      dest, ctx->path);
    return 1;
  }
  ...
  do
  {
    ...
    if (!rc)
    {
      mutt_log (1, "imap_fast_trash: No messages to trash\n");
      rc = -1;
      goto out;
    }
    else if (rc < 0)
    {
      mutt_log (1, "could not queue copy\n");
      goto out;
    }
    else
    ...
    if (rc == -2)
    {
      if (triedcreate)
      {
        mutt_log (1, "Already tried to create mailbox %s\n", mbox);
        break;
      }
      /* bail out if command failed for reasons other than nonexistent target */
      if (ascii_strncasecmp (imap_get_qualifier (idata->buf), "[TRYCREATE]", 11))
        break;
      mutt_log (3, "imap_fast_trash: server suggests TRYCREATE\n");
