...
imap_auth_res_t imap_auth_sasl (IMAP_DATA* idata, const char* method)
{
  ...
  {
    mutt_log (1, "imap_auth_sasl: Error allocating SASL connection.\n");
  ...
  {
    if (method)
      mutt_log (2, "imap_auth_sasl: %s unavailable\n", method);
    else
      mutt_log (1, "imap_auth_sasl: Failure starting authentication exchange. No shared mechanisms?\n");
  ...
  if (mutt_bit_isset (idata->capabilities, SASL_IR) && client_start)
  {
    ...
    {
      mutt_log (1, "imap_auth_sasl: error base64-encoding client response.\n");
      goto bail;
    }
    client_start = olen = 0;
  }
  ...
  while (rc == SASL_CONTINUE || olen > 0)
  {
	...
	len = 0;
      ...
      else
      {
        ...
        {
          mutt_log (1, "imap_auth_sasl: error base64-decoding server response.\n");
          goto bail;
        }
      }
    }
    ...
    if (rc < 0)
    {
      ...
      mutt_log (1, "imap_auth_sasl: sasl_client_step error %d\n",rc);
    }
 ...
 bail:
  ...
  if (method)
  {
    mutt_log (2, "imap_auth_sasl: %s failed\n", method);
    return IMAP_AUTH_UNAVAIL;
  }
