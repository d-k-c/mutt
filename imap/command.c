...
int imap_cmd_step (IMAP_DATA* idata)
{
  ...
  do
  {
    if (len == idata->blen)
    {
      ...
      mutt_log (3, "imap_cmd_step: grew buffer to %u bytes\n",
		  idata->blen);
    }
    ...
    if (c <= 0)
    {
      mutt_log (1, "imap_cmd_step: Error reading server response.\n");
      cmd_handle_fatal (idata);
      return IMAP_CMD_BAD;
    }
  ...
  if ((idata->blen > IMAP_CMD_BUFSIZE) && (len <= IMAP_CMD_BUFSIZE))
  {
    ...
    mutt_log (3, "imap_cmd_step: shrank buffer to %u bytes\n", idata->blen);
  }
...
const char* imap_cmd_trailer (IMAP_DATA* idata)
{
  ...
  if (!s)
  {
    mutt_log (2, "imap_cmd_trailer: not a tagged response");
    return notrailer;
  }
  ...
  {
    mutt_log (2, "imap_cmd_trailer: not a command completion: %s",
		idata->buf);
    return notrailer;
  }
...
int imap_exec (IMAP_DATA* idata, const char* cmdstr, int flags)
{
  ...
  if (rc != IMAP_CMD_OK)
  {
    ...
    mutt_log (1, "imap_exec: command failed: %s\n", idata->buf);
    return -1;
  }
...
void imap_cmd_finish (IMAP_DATA* idata)
{
  ...
  if (idata->reopen & IMAP_REOPEN_ALLOW)
  {
	...
	&& count > idata->ctx->msgcount)
    {
      ...
      mutt_log (2, "imap_cmd_finish: Fetching new mail\n");
    ...
    {
      mutt_log (2, "imap_cmd_finish: Expunging mailbox\n");
...
int imap_cmd_idle (IMAP_DATA* idata)
{
  ...
  if (rc != IMAP_CMD_OK)
  {
    mutt_log (1, "imap_cmd_idle: error starting IDLE\n");
    return -1;
  }
...
static IMAP_COMMAND* cmd_new (IMAP_DATA* idata)
{
  ...
  {
    mutt_log (3, "cmd_new: IMAP command queue full\n");
    return NULL;
  }
...
static int cmd_queue (IMAP_DATA* idata, const char* cmdstr)
{
  ...
  if (cmd_queue_full (idata))
  {
    mutt_log (3, "Draining IMAP command pipeline\n");
...
static int cmd_handle_untagged (IMAP_DATA* idata)
{
  ...
  if ((idata->state >= IMAP_SELECTED) && isdigit ((unsigned char) *s))
  {
    ...
    if (ascii_strncasecmp ("EXISTS", s, 6) == 0)
    {
	   ...
	   count < idata->ctx->msgcount)
      {
         ...
         * it should. */
	mutt_log (1, "Message count is out of sync");
	return 0;
      }
  ...
  {
    mutt_log (2, "Handling untagged NO\n");
...
static void cmd_parse_capability (IMAP_DATA* idata, char* s)
{
  ...
  mutt_log (3, "Handling CAPABILITY\n");
...
static void cmd_parse_fetch (IMAP_DATA* idata, char* s)
{
  ...
  /* see cmd_parse_expunge */
    for (cur = 0; cur < idata->ctx->msgcount; cur++)
    {
      ...
      {
	mutt_log (2, "Message UID %d updated\n", HEADER_DATA(h)->uid);
	break;
      }
  ...
  if (!h)
  {
    mutt_log (3, "FETCH response ignored for this message\n");
    return;
  }
  ...
  if (*s != '(')
  {
    mutt_log (1, "Malformed FETCH response");
    return;
  }
  s++;
  ...
  {
    mutt_log (2, "Only handle FLAGS updates\n");
    return;
  }
...
static void cmd_parse_status (IMAP_DATA* idata, char* s)
{
  ...
  if (*s++ != '(')
  {
    mutt_log (1, "Error parsing STATUS\n");
    return;
  }
  while (*s && *s != ')')
  {
  ...
  mutt_log (3, "%s (UIDVALIDITY: %d, UIDNEXT: %d) %d messages, %d recent, %d unseen\n",
  ...
  mutt_log (3, "Running default STATUS handler\n");
  ...
  for (inc = Incoming; inc; inc = inc->next)
  {
    ...
    {
      mutt_log (1, "Error parsing mailbox %s, skipping\n", inc->path);
      continue;
    }
    /* mutt_log (2, "Buffy entry: [%s] mbox: [%s]\n", inc->path, NONULL(mx.mbox)); */
	...
	value = safe_strdup ("INBOX");
      ...
      {
        mutt_log (3, "Found %s in buffy list (OV: %d ON: %d U: %d)\n",
