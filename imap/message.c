...
int imap_read_headers (IMAP_DATA* idata, int msgbegin, int msgend)
{
  ...
  if (evalhc)
  {
        ...
        else if (mfhrc < 0)
	{
	  imap_free_header_data (&h.data);
        ...
        if (!h.data->uid)
        {
          mutt_log (2, "imap_read_headers: skipping hcache FETCH "
	  ...
	  /* bad header in the cache, we'll have to refetch. */
          mutt_log (3, "bad cache entry at %d, giving up\n", h.sid - 1);
          imap_free_header_data(&h.data);
          evalhc = 0;
          idx--;
        }
  ...
  for (msgno = msgbegin; msgno <= msgend ; msgno++)
  {
	...
	break;
      ...
      if (!ftello (fp))
      {
        mutt_log (2, "msg_fetch_header: ignoring fetch response with no body\n");
        mfhrc = -1;
        msgend--;
        continue;
      }
      ...
      if (idx > msgend)
      {
        mutt_log (1, "imap_read_headers: skipping FETCH response for "
      ...
      if (idx < ctx->msgcount)
      {
	mutt_log (2, "imap_read_headers: message %d is not new\n",
		    h.sid);
        idx--;
	continue;
      }
  ...
  {
    mutt_log (2, "Overriding UIDNEXT: %u -> %u\n", idata->uidnext, maxuid + 1);
    idata->uidnext = maxuid + 1;
  }
  if (idata->uidnext > 1)
...
int imap_append_message (CONTEXT *ctx, MESSAGE *msg)
{
  ...
  if (rc != IMAP_CMD_RESPOND)
  {
    ...
    mutt_log (1, "imap_append_message(): command failed: %s\n",
		idata->buf);
...
int imap_copy_messages (CONTEXT* ctx, HEADER* h, char* dest, int delete)
{
  ...
  {
    mutt_log (1, "imap_copy_messages: bad destination %s\n", dest);
    return -1;
  }
  ...
  {
    mutt_log (3, "imap_copy_messages: %s not same server as %s\n",
      dest, ctx->path);
    return 1;
  }
  ...
  if (h && h->attach_del)
  {
    mutt_log (3, "imap_copy_messages: Message contains attachments to be deleted\n");
    return 1;
  }
  ...
  do
  {
    ...
    if (!h)
    {
      ...
      for (n = 0; n < ctx->msgcount; n++)
      {
            ...
            ctx->hdrs[n]->changed)
        {
          ...
          if (rc < 0)
          {
            mutt_log (1, "imap_copy_messages: could not sync\n");
            goto out;
          }
        }
      }
      ...
      if (!rc)
      {
        mutt_log (1, "imap_copy_messages: No messages tagged\n");
        rc = -1;
        goto out;
      }
      else if (rc < 0)
    ...
    if (rc == -2)
    {
      ...
      if (ascii_strncasecmp (imap_get_qualifier (idata->buf), "[TRYCREATE]", 11))
        break;
      mutt_log (3, "imap_copy_messages: server suggests TRYCREATE\n");
...
char* imap_set_flags (IMAP_DATA* idata, HEADER* h, char* s)
{
  ...
  mutt_log (2, "imap_fetch_message: parsing FLAGS\n");
...
static int msg_parse_fetch (IMAP_HEADER *h, char *s)
{
  ...
  while (*s)
  {
    ...
    else if (ascii_strncasecmp ("INTERNALDATE", s, 12) == 0)
    {
      ...
      if (*s != '\"')
      {
        mutt_log (1, "msg_parse_fetch(): bogus INTERNALDATE entry: %s\n", s);
        return -1;
      }
      s++;
      ptmp = tmp;
...
static char* msg_parse_flags (IMAP_HEADER* h, char* s)
{
  ...
  if (ascii_strncasecmp ("FLAGS", s, 5) != 0)
  {
    mutt_log (1, "msg_parse_flags: not a FLAGS response: %s\n",
      s);
    return NULL;
  }
  s += 5;
  ...
  if (*s != '(')
  {
    mutt_log (1, "msg_parse_flags: bogus FLAGS response: %s\n",
      s);
    return NULL;
  }
  s++;
  ...
  {
    mutt_log (1, "msg_parse_flags: Unterminated FLAGS response: %s\n", s);
    return NULL;
  }
