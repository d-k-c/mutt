...
int imap_browse (char* path, struct browser_state* state)
{
  ...
  if (n)
  {
    ...
    if (n > 0)			/* "aaaa/bbbb/" -> "aaaa" */
    {
      ...
      if (showparents)
      {
	mutt_log (3, "imap_init_browse: adding parent %s\n", mbox);
  ...
  mutt_log (3, "imap_browse: Quoting mailbox scan: %s -> ", mbox);
...
int imap_mailbox_create (const char* folder)
{
  ...
  {
    mutt_log (1, "imap_mailbox_create: Bad starting path %s\n",
      folder);
    return -1;
  }
  ...
  {
    mutt_log (1, "imap_mailbox_create: Couldn't find open connection to %s", mx.account.host);
    goto fail;
  }
...
int imap_mailbox_rename(const char* mailbox)
{
  ...
  {
    mutt_log (1, "imap_mailbox_rename: Bad source mailbox %s\n",
      mailbox);
    return -1;
  }
  ...
  if (!(idata = imap_conn_find (&mx.account, MUTT_IMAP_CONN_NONEW)))
  {
    mutt_log (1, "imap_mailbox_rename: Couldn't find open connection to %s", mx.account.host);
    goto fail;
  }
  ...
  struct browser_state* state, short isparent)
{
  ...
  {
    mutt_log (2, "browse_add_list_result: current folder %s makes no sense\n",
              state->folder);
    return -1;
  }
