...
static void print_gss_error(OM_uint32 err_maj, OM_uint32 err_min)
{
	...
	mutt_log (2, "((%s:%d )(%s:%d))", buf_maj, err_maj, buf_min, err_min);
}
...
imap_auth_res_t imap_auth_gss (IMAP_DATA* idata, const char* method)
{
  ...
  else if (mutt_log_get_level () >= 2)
  {
    maj_stat = gss_display_name (&min_stat, target_name, &request_buf,
      &mech_name);
    mutt_log (2, "Using service name [%s]\n",
  ...
  if (maj_stat != GSS_S_COMPLETE && maj_stat != GSS_S_CONTINUE_NEEDED)
  {
    ...
    mutt_log (1, "Error acquiring credentials - no TGT?\n");
  ...
  {
    mutt_log (2, "Invalid response from server: %s\n", buf1);
    gss_release_name (&min_stat, &target_name);
    goto bail;
  }

  /* now start the security context initialisation loop... */
  mutt_log (2, "Sending credentials\n");
  ...
  while (maj_stat == GSS_S_CONTINUE_NEEDED)
  {
    ...
    if (rc != IMAP_CMD_RESPOND)
    {
      mutt_log (1, "Error receiving server response.\n");
      gss_release_name (&min_stat, &target_name);
      goto bail;
    }
    ...
    {
      print_gss_error(maj_stat, min_stat);
      mutt_log (1, "Error exchanging credentials\n");
      gss_release_name (&min_stat, &target_name);
  ...
  {
    print_gss_error(maj_stat, min_stat);
    mutt_log (2, "Couldn't unwrap security level data\n");
    gss_release_buffer (&min_stat, &send_token);
    goto err_abort_cmd;
  }
  ...
  gss_release_buffer (&min_stat, &send_token);
  mutt_log (2, "Unwrapped security level flags: %c%c%c\n",
    server_conf_flags & GSS_AUTH_P_NONE      ? 'N' : '-',
    server_conf_flags & GSS_AUTH_P_INTEGRITY ? 'I' : '-',
    server_conf_flags & GSS_AUTH_P_PRIVACY   ? 'P' : '-');
  mutt_log (2, "Maximum GSS token size is %ld\n", buf_size);
  ...
  if (maj_stat != GSS_S_COMPLETE)
  {
    mutt_log (2, "Error creating login request\n");
    goto err_abort_cmd;
  }

  mutt_to_base64 ((unsigned char*) buf1, send_token.value, send_token.length,
		  sizeof (buf1) - 2);
  mutt_log (2, "Requesting authorisation as %s\n",
  ...
  if (rc == IMAP_CMD_RESPOND)
  {
    mutt_log (1, "Unexpected server continuation request.\n");
    goto err_abort_cmd;
  }
  if (imap_code (idata->buf))
  {
    /* flush the security context */
    mutt_log (2, "Releasing GSS credentials\n");
    maj_stat = gss_delete_sec_context (&min_stat, &context, &send_token);
    if (maj_stat != GSS_S_COMPLETE)
      mutt_log (1, "Error releasing credentials\n");
