...
static int pop_read_header (POP_DATA *pop_data, HEADER *h)
{
  ...
  if (ret == 0)
  {
	...
	mutt_log (1, "pop_read_header: set TOP capability\n");
      }
	...
	mutt_log (1, "pop_read_header: unset TOP capability\n");
...
static int fetch_uidl (char *line, void *data)
{
  ...
  if (i == ctx->msgcount)
  {
    mutt_log (1, "pop_fetch_headers: new header %d %s\n", index, line);
...
static int pop_fetch_headers (CONTEXT *ctx)
{
  ...
  if (pop_data->cmd_uidl == 2)
  {
    if (ret == 0)
    {
      ...
      mutt_log (1, "pop_fetch_headers: set UIDL capability\n");
    }
    ...
    if (ret == -2 && pop_data->cmd_uidl == 2)
    {
      ...
      mutt_log (1, "pop_fetch_headers: unset UIDL capability\n");
      snprintf (pop_data->err_msg, sizeof (pop_data->err_msg),
...
static void pop_clear_cache (POP_DATA *pop_data)
{
  ...
  mutt_log (1, "pop_clear_cache: delete cached messages\n");
