...
{
  int err;
  mutt_log (1, "getnameinfo: ");
  switch(ret)
  {
     case EAI_AGAIN:
       mutt_log (1, "The name could not be resolved at this time.  Future attempts may succeed.\n");
       err=SASL_TRYAGAIN;
       break;
     case EAI_BADFLAGS:
       mutt_log (1, "The flags had an invalid value.\n");
       err=SASL_BADPARAM;
       break;
     case EAI_FAIL:
       mutt_log (1, "A non-recoverable error occurred.\n");
       err=SASL_FAIL;
       break;
     case EAI_FAMILY:
       mutt_log (1, "The address family was not recognized or the address length was invalid for the specified family.\n");
       err=SASL_BADPROT;
       break;
     case EAI_MEMORY:
       mutt_log (1, "There was a memory allocation failure.\n");
       err=SASL_NOMEM;
       break;
     case EAI_NONAME:
       mutt_log (1, "The name does not resolve for the supplied parameters.  NI_NAMEREQD is set and the host's name cannot be located, or both nodename and servname were null.\n");
       err=SASL_FAIL; /* no real equivalent */
       break;
     case EAI_SYSTEM:
       mutt_log (1, "A system error occurred.  The error code can be found in errno(%d,%s)).\n",errno,strerror(errno));
       err=SASL_FAIL; /* no real equivalent */
       break;
     default:
       mutt_log (1, "Unknown error %d\n",ret);
       err=SASL_FAIL; /* no real equivalent */
       break;
  }
  return err;
...
int mutt_sasl_start (void)
{
  ...
  if (rc != SASL_OK)
  {
    mutt_log (1, "mutt_sasl_start: libsasl initialisation failed.\n");
    return SASL_FAIL;
  }
...
int mutt_sasl_client_new (CONNECTION* conn, sasl_conn_t** saslconn)
{
  ...
  if (!getsockname (conn->fd, (struct sockaddr *)&local, &size)) {
    if (iptostring((struct sockaddr *)&local, size, iplocalport,
      ...
      mutt_log (2, "SASL failed to parse local IP address\n");
  }
  else
    mutt_log (2, "SASL failed to get local IP address\n");
  ...
  if (!getpeername (conn->fd, (struct sockaddr *)&remote, &size)){
    if (iptostring((struct sockaddr *)&remote, size, ipremoteport,
      ...
      mutt_log (2, "SASL failed to parse remote IP address\n");
  }
  else
    mutt_log (2, "SASL failed to get remote IP address\n");

  mutt_log (2, "SASL local ip: %s, remote ip:%s\n", NONULL(plp),
	      NONULL(prp));
  ...
  {
    mutt_log (2, "External authentication name: %s\n", conn->account.user);
...
int mutt_sasl_interact (sasl_interact_t* interaction)
{
  ...
  {
    mutt_log (2, "mutt_sasl_interact: filling in SASL interaction %ld.\n", interaction->id);
...
void mutt_sasl_setup_conn (CONNECTION* conn, sasl_conn_t* saslconn)
{
  ...
  sasldata->ssf = tmp;
  mutt_log (3, "SASL protection strength: %u\n", *sasldata->ssf);
  /* Add SASL SSF to transport SSF */
  conn->ssf += *sasldata->ssf;
  sasl_getprop (saslconn, SASL_MAXOUTBUF, &tmp);
  sasldata->pbufsize = tmp;
  mutt_log (3, "SASL protection buffer size: %u\n", *sasldata->pbufsize);
  ...
  unsigned* len)
{
  ...
  *result = NULL;
  ...
  mutt_log (2, "mutt_sasl_cb_authname: getting %s for %s:%u\n",
  ...
  sasl_secret_t** psecret)
{
  ...
  mutt_log (2, "mutt_sasl_cb_pass: getting password for %s@%s:%u\n",
...
static int mutt_sasl_conn_read (CONNECTION* conn, char* buf, size_t len)
{
  ...
  if (*sasldata->ssf)
  {
	...
	goto out;
      ...
      if (rc != SASL_OK)
      {
	mutt_log (1, "SASL decode failed: %s\n",
  ...
  size_t len)
{
  ...
  if (*sasldata->ssf)
  {
    ...
    do
    {
      ...
      if (rc != SASL_OK)
      {
	mutt_log (1, "SASL encoding failed: %s\n",
          sasl_errstring (rc, NULL, NULL));
	goto fail;
      }
