...
static void pgp_dearmor (FILE *in, FILE *out)
{
  ...
  if (r == NULL)
  {
    mutt_log (1, "pgp_dearmor: Can't find begin of ASCII armor.\n");
    return;
  }
  ...
  if (r == NULL)
  {
    mutt_log (1, "pgp_dearmor: Armor header doesn't end.\n");
    return;
  }
  ...
  if (r == NULL)
  {
    mutt_log (1, "pgp_dearmor: Can't find end of ASCII armor.\n");
    return;
  }
  
  if ((end = ftello (in) - strlen (line)) < start)
  {
    mutt_log (1, "pgp_dearmor: end < start???\n");
    return;
  }
  
  if (fseeko (in, start, SEEK_SET) == -1)
  {
    mutt_log (1, "pgp_dearmor: Can't seekto start.\n");
    return;
  }
...
static short pgp_mic_from_packet (unsigned char *p, size_t len)
{
  ...
  if ((p[0] & 0x3f) != PT_SIG)
  {
    mutt_log (1, "pgp_mic_from_packet: tag = %d, want %d.\n",
		p[0]&0x3f, PT_SIG);
    return -1;
  }
  ...
  {
    mutt_log (1, "pgp_mic_from_packet: Bad signature packet.\n");
    return -1;
  }
}
...
static short pgp_find_hash (const char *fname)
{
  ...
  {
    mutt_log (1, "pgp_find_hash: No packet.\n");
  }
