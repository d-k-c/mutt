...
static void tls_get_client_cert (CONNECTION* conn)
{
  ...
  {
    mutt_log (1, "Failed to init gnutls crt\n");
    return;
  }
  if (gnutls_x509_crt_import (clientcrt, crtdata, GNUTLS_X509_FMT_DER) < 0)
  {
    mutt_log (1, "Failed to import gnutls client crt\n");
    goto err_crt;
  }
  /* get length of DN */
  dnlen = 0;
  ...
  {
    mutt_log (1, "could not allocate DN\n");
    goto err_crt;
  }
  gnutls_x509_crt_get_dn (clientcrt, dn, &dnlen);
  mutt_log (2, "client certificate DN: %s\n", dn);

  /* extract CN to use as external user name */
  if (!(cn = strstr (dn, "CN=")))
  {
    mutt_log (1, "no CN found in DN\n");
    goto err_dn;
  }
  cn += 3;
  ...
  for (i = cert_list_size - 1; i >= 0; i--)
  {
    ...
    if (i && rc) {
      rc = gnutls_certificate_set_x509_trust_mem (data->xcred, &cert_list[i],
                                                  GNUTLS_X509_FMT_DER);
      if (rc != 1)
        mutt_log (1, "error trusting certificate %d: %d\n", i, rc);
