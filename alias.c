...
static ADDRESS *mutt_expand_aliases_r (ADDRESS *a, LIST **expn)
{
  ...
  while (a)
  {
        ...
        for (u = *expn; u; u = u->next)
	{
	  ...
	  {
	    mutt_log (1, "mutt_expand_aliases_r(): loop in alias found for '%s'\n", a->mailbox);
	    i = 1;
	    break;
	  }
	}
...
int mutt_addr_is_user (ADDRESS *addr)
{
  ...
  if (!addr)
  {
    mutt_log (5, "mutt_addr_is_user: yes, NULL address\n");
    return 1;
  }
  if (!addr->mailbox)
  {
    mutt_log (5, "mutt_addr_is_user: no, no mailbox\n");
    return 0;
  }

  if (ascii_strcasecmp (addr->mailbox, Username) == 0)
  {
    mutt_log (5, "mutt_addr_is_user: yes, %s = %s\n", addr->mailbox, Username);
    return 1;
  }
  if (string_is_address(addr->mailbox, Username, Hostname))
  {
    mutt_log (5, "mutt_addr_is_user: yes, %s = %s @ %s \n", addr->mailbox, Username, Hostname);
    return 1;
  }
  fqdn = mutt_fqdn (0);
  ...
  {
    mutt_log (5, "mutt_addr_is_user: yes, %s matched by alternates.\n", addr->mailbox);
    if (mutt_match_rx_list (addr->mailbox, UnAlternates))
      mutt_log (5, "mutt_addr_is_user: but, %s matched by unalternates.\n", addr->mailbox);
    else
      return 1;
  }
  
  mutt_log (5, "mutt_addr_is_user: no, all failed.\n");
  return 0;
}
