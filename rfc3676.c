			       ...
			       flowed_state_t *fst, int term)
{
  ...
  mutt_log (4, "f=f: line [%s], width = %ld, spaces = %d\n",
	      NONULL(line), (long)width, fst->spaces);

  for (p = (char *)line, words = 0; (p = strsep (&line, " ")) != NULL ; )
  {
    mutt_log (4, "f=f: word [%s], width: %d, remaining = [%s]\n",
	      p, fst->width, line);

    /* remember number of spaces */
    if (!*p)
    {
      mutt_log (4, "f=f: additional space\n");
      fst->spaces++;
      continue;
    }
    ...
    {
      mutt_log (4, "f=f: break line at %d, %d spaces left\n",
