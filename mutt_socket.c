...
int mutt_socket_open (CONNECTION* conn) 
{
  ...
  mutt_log (2, "Connected to %s:%d on fd=%d\n",
...
int mutt_socket_close (CONNECTION* conn)
{
  ...
  if (conn->fd < 0)
    mutt_log (1, "mutt_socket_close: Attempt to close closed connection.\n");
  else
...
int mutt_socket_read (CONNECTION* conn, char* buf, size_t len)
{
  ...
  if (conn->fd < 0)
  {
    mutt_log (1, "mutt_socket_read: attempt to read from closed connection\n");
    return -1;
  }
...
int mutt_socket_write_d (CONNECTION *conn, const char *buf, int len, int dbg)
{
  ...
  mutt_log (dbg, "%d> %s", conn->fd, buf);

  if (conn->fd < 0)
  {
    mutt_log (1, "mutt_socket_write: attempt to write to closed connection\n");
    return -1;
  }
  ...
  while (sent < len)
  {
    ...
    {
      mutt_log (1, "mutt_socket_write: error writing (%s), closing socket\n",
                strerror(errno));
      mutt_socket_close (conn);
    ...
    if (rc < len - sent)
      mutt_log (3, "mutt_socket_write: short write (%d of %d bytes)\n",
                rc, len - sent);
...
int mutt_socket_readchar (CONNECTION *conn, char *c)
{
  if (conn->bufpos >= conn->available)
  {
    ...
    {
      mutt_log (1, "mutt_socket_readchar: attempt to read from closed connection.\n");
      return -1;
    }
    conn->bufpos = 0;
    if (conn->available == 0)
...
static int socket_preconnect (void)
{
  ...
  {
    mutt_log (2, "Executing preconnect: %s\n", Preconnect);
    rc = mutt_system (Preconnect);
    mutt_log (2, "Preconnect result: %d\n", rc);
    if (rc)
    {
      save_errno = errno;
...
static int socket_connect (int fd, struct sockaddr* sa)
{
  ...
  {
    mutt_log (1, "Unknown address family!\n");
    return -1;
  }
  ...
  {
      save_errno = errno;
      mutt_log (2, "Connection failed. errno: %d...\n", errno);
