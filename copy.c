	       ...
	       const char *prefix)
{
  ...
  if (flags & CH_REORDER)
  {
    ...
    {
      mutt_log (3, "Reorder list: %s\n", t->data);
      hdr_count++;
    }
  }

  mutt_log (1, "WEED is %s\n", (flags & CH_WEED) ? "Set" : "Not");
  ...
  while (ftello (in) < off_end)
  {
	...
	for (t = HeaderOrderList, x = 0 ; (t) ; t = t->next, x++)
	{
	  ...
	  {
	    mutt_log (2, "Reorder: %s matches %s\n", t->data, buf);
	    break;
	  }
	}
      }
    ...
    if (!ignore)
    {
      mutt_log (2, "Reorder: x = %d; hdr_count = %d\n", x, hdr_count);
      if (!this_one) {
		   ...
		   int chflags)
{
  ...
  {
    mutt_log (1, "_mutt_copy_message failed to detect EOF!\n");
    r = -1;
  }
