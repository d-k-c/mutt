...
int mx_lock_file (const char *path, int fd, int excl, int dot, int timeout)
{
  ...
  {
    mutt_log (1, "mx_lock_file(): fcntl errno %d.\n", errno);
...
int mx_get_magic (const char *path)
{
  ...
  if (stat (path, &st) == -1)
  {
    mutt_log (1, "mx_get_magic(): unable to stat %s: %s (errno %d).\n",
  ...
  {
    mutt_log (1, "mx_get_magic(): unable to open file %s for reading.\n",
		path);
    return (-1);
  }
...
MESSAGE *mx_open_new_message (CONTEXT *dest, HEADER *hdr, int flags)
{
  ...
  {
      mutt_log (1, "mx_open_new_message(): function unimplemented for mailbox type %d.\n",
              dest->magic);
      return NULL;
  }
...
int mx_check_mailbox (CONTEXT *ctx, int *index_hint)
{
  if (!ctx)
  {
    mutt_log (1, "mx_check_mailbox: null or invalid context.\n");
    return -1;
  }
...
MESSAGE *mx_open_message (CONTEXT *ctx, int msgno)
{
  ...
  {
    mutt_log (1, "mx_open_message(): function not implemented for mailbox type %d.\n", ctx->magic);
    return NULL;
  }
...
int mx_commit_message (MESSAGE *msg, CONTEXT *ctx)
{
  ...
  {
    mutt_log (1, "mx_commit_message(): msg->write = %d, ctx->append = %d\n",
...
int mx_close_message (CONTEXT *ctx, MESSAGE **msg)
{
  ...
  if ((*msg)->path)
  {
    mutt_log (1, "mx_close_message (): unlinking %s\n",
              (*msg)->path);
    unlink ((*msg)->path);
    FREE (&(*msg)->path);
  }
