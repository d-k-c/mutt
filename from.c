...
int is_from (const char *s, char *path, size_t pathlen, time_t *tp)
{
  ...
  if (!is_day_name (s))
  {
    ...
    if (ascii_strncasecmp(p, " at ", 4) == 0)
    {
      ...
      if (!p)
      {
	mutt_log (1, "is_from(): error parsing what appears to be a pipermail-style obscured return_path: %s\n", s);
	return 0;
      }
    }
	...
	len = pathlen - 1;
      memcpy (path, s, len);
      path[len] = 0;
      mutt_log (3, "is_from(): got return path: %s\n", path);
    }
    ...
    if (!is_day_name (s))
    {
      mutt_log (1, "is_from():  expected weekday, got: %s\n", s);
      return 0;
    }
  }
  ...
  mutt_log (3, "is_from(): month=%d, day=%d, hr=%d, min=%d, sec=%d, yr=%d.\n",
