  ...
  if (*buf_ready == 0)
  {
    ...
    *buf_ready = 1;
	...
	while (*p++ != 'm')	/* skip ANSI sequence */
      ...
      {
	mutt_log (2, "fill_buffer: Seen attachment marker.\n");
                        ...
                        mutt_window_t *pager_window)
{
  ...
  for (ch = 0, vch = 0; ch < cnt; ch += k, vch += k)
  {
    ...
    if (k == (size_t)(-2) || k == (size_t)(-1))
    {
      if (k == (size_t)(-1))
        memset(&mbstate, 0, sizeof(mbstate));
      mutt_log (1, "%s:%d: mbrtowc returned %d; errno = %d.\n",
    ...
    if (Charset_is_utf8)
    {
      ...
      {
	mutt_log (3, "skip zero-width character U+%04X\n", (unsigned short)wc);
	continue;
      }
      if (is_display_corrupting_utf8 (wc))
      {
	mutt_log (3, "skip bidi marker U+%04X\n", (unsigned short)wc);
	continue;
      }
    }
