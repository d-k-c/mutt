...
static pop_auth_res_t pop_auth_sasl (POP_DATA *pop_data, const char *method)
{
  ...
  {
    mutt_log (1, "pop_auth_sasl: Error allocating SASL connection.\n");
  ...
  {
    mutt_log (1, "pop_auth_sasl: Failure starting authentication exchange. No shared mechanisms?\n");
  ...
  FOREVER
  {
    ...
    {
      mutt_log (1, "pop_auth_sasl: error base64-decoding server response.\n");
      goto bail;
    }
    ...
    if (pc)
    {
      ...
      {
	mutt_log (1, "pop_auth_sasl: error base64-encoding client response.\n");
	goto bail;
      }
    }
  }
...
static pop_auth_res_t pop_auth_user (POP_DATA *pop_data, const char *method)
{
  ...
  if (pop_data->cmd_user == 2)
  {
    if (ret == 0)
    {
      ...
      mutt_log (1, "pop_auth_user: set USER capability\n");
    }
    ...
    if (ret == -2)
    {
      ...
      mutt_log (1, "pop_auth_user: unset USER capability\n");
...
int pop_authenticate (POP_DATA* pop_data)
{
  ...
  if (PopAuthenticators && *PopAuthenticators)
  {
    ...
    while (method)
    {
      ...
      if (comma)
	*comma++ = '\0';
      mutt_log (2, "pop_authenticate: Trying method %s\n", method);
  ...
  else
  {
    ...
    mutt_log (2, "pop_authenticate: Using any available method.\n");
    authenticator = pop_authenticators;
