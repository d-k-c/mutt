...
int mutt_compose_attachment (BODY *a)
{
  ...
  if (rfc1524_mailcap_lookup (a, type, entry, MUTT_COMPOSE))
  {
	...
	strfcpy (command, entry->composecommand, sizeof (command));
      ...
      {
	mutt_log (1, "oldfile: %s\t newfile: %s\n",
				  a->filename, newfile);
  ...
  for (; t; t = t->next) {
    ...
    if ((n = mutt_lookup_mime_type (&tmp, b->filename)) != TYPEOTHER) {
      snprintf (type, len, "%s/%s",
                ...
                tmp.subtype);
      mutt_log (1, "mutt_check_lookup_list: \"%s\" -> %s\n",
                b->filename, type);
    }
    if (tmp.subtype) 
      FREE (&tmp.subtype);
			  ...
			  ATTACHPTR **idx, short idxlen)
{
  ...
  else
  {
	...
	{
	  mutt_log (1, "mutt_view_attachment:%d safe_fopen(%s) errno=%d %s\n", __LINE__, pagerfile, errno, strerror(errno));
	...
	if (fclose(decode_state.fpout) == EOF)
	  mutt_log (1, "mutt_view_attachment:%d fclose errno=%d %s\n", __LINE__, pagerfile, errno, strerror(errno));
      }
      else
      {
...
int mutt_print_attachment (FILE *fp, BODY *a)
{
  ...
  else if (mutt_can_decode (a))
  {
    ...
    if (mutt_decode_save_attachment (fp, a, newfile, MUTT_PRINTING, 0) == 0)
    {
      ...
      mutt_log (2, "successfully decoded %s type attachment to %s\n",
		  type, newfile);
	...
	goto bail0;
      ...
      mutt_log (2, "successfully opened %s read-only\n", newfile);
