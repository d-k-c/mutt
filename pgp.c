...
static int pgp_copy_checksig (FILE *fpin, FILE *fpout)
{
  ...
  if (PgpGoodSign.pattern)
  {
    ...
    while ((line = mutt_read_line (line, &linelen, fpin, &lineno, 0)) != NULL)
    {
      ...
      {
	mutt_log (2, "pgp_copy_checksig: \"%s\" matches regexp.\n",
		    line);
	rv = 0;
      }
      else
	mutt_log (2, "pgp_copy_checksig: \"%s\" doesn't match regexp.\n",
		    line);
  ...
  {
    mutt_log (2, "pgp_copy_checksig: No pattern.\n");
...
static int pgp_check_decryption_okay (FILE *fpin)
{
  ...
  if (PgpDecryptionOkay.pattern)
  {
    ...
    while ((line = mutt_read_line (line, &linelen, fpin, &lineno, 0)) != NULL)
    {
      ...
      {
        mutt_log (2, "pgp_check_decryption_okay: \"%s\" matches regexp.\n",
                    line);
        rv = 0;
        break;
      }
      else
        mutt_log (2, "pgp_check_decryption_okay: \"%s\" doesn't match regexp.\n",
                    line);
    }
    FREE (&line);
  }
  ...
  {
    mutt_log (2, "pgp_check_decryption_okay: No pattern.\n");
    rv = 1;
  }
...
int pgp_application_pgp_handler (BODY *m, STATE *s)
{
  ...
  for (bytes = m->length; bytes > 0;)
  {
	...
	mutt_log (4, "pgp: recoding inline from [%s] to [%s]\n",
...
int pgp_verify_one (BODY *sigbdy, STATE *s, const char *tempfile)
{
				   ...
				   tempfile, sigfile)) != -1)
  {
    ...
    if ((rv = mutt_wait_filter (thepid)))
     ...
     mutt_log (1, "pgp_verify_one: mutt_wait_filter returned %d.\n", rv);
  }
  ...
  mutt_log (1, "pgp_verify_one: returning %d.\n", badsig);
